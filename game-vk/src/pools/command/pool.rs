...
impl CommandPool {
    ...
    pub fn allocate(&mut self, index: u32, flags: CommandBufferFlags, level: CommandBufferLevel) -> Result<Rc<CommandBuffer>, Error> {
        ...
        let pool: vk::CommandPool = match pools.get(&flags) {
            Some(pool) => *pool,
            None       => {
    ...
    /// 
    /// It will panic if the given queue family index is not in the user queue families when this pool was created.
    pub fn n_allocate(&mut self, count: u32, index: u32, flags: CommandBufferFlags, level: CommandBufferLevel) -> Result<Vec<Rc<CommandBuffer>>, Error> {
        // Insert a pool with these specs if it does not yet exist
        let pools: &mut HashMap<CommandBufferFlags, vk::CommandPool> = self.pools.get_mut(&index).unwrap_or_else(|| panic!("Unknown queue family index '{}'", index));
        let pool: vk::CommandPool = match pools.get(&flags) {
            Some(pool) => *pool,
